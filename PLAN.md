...
6. ✅ **v0.6 — daemon-ready** (для системных агентов à la Zabbix/UEM):
   OnReload + host.Reload() (SIGHUP на unix, PARAMCHANGE под SCM),
   `shost/sdnotify` (systemd Type=notify: READY/STOPPING/watchdog +
   генератор unit-файла, zero-dep), `shost/single` (единственный экземпляр:
   flock / эксклюзивный handle, zero-dep), отдельный модуль `shost/winsvc`
   (аналог UseWindowsService: SCM handshake, STOP_PENDING checkpoints,
   Event Log, Install/Uninstall; на не-Windows — фолбэк в host.Run).
   Агентские вещи (коллекторы, транспорт, офлайн-буфер, self-update) —
   кандидаты в отдельную библиотеку sagent поверх shost, не в ядро.
7. **Docs**: страница shost на dvislobokov.github.io в общем стиле, README с
