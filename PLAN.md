...
2. **v0.2 — lifecycle+**: события Started/Stopping/Stopped, политики рестарта
   с backoff, опциональный интерфейс готовности (Readier) + WithStartTimeout —
   т.к. Start блокирующий, барьер «сервис готов» требует отдельного сигнала.
   (`Shutdown()` реализован уже в v0.1.)
3. **v0.3 — экосистема**: environments + sconf-хелпер, `httpsvc`, `cron`, `health`
   с /healthz и /readyz.
4. **v0.4 — наблюдаемость**: `shost/otel` (метрики + трейсы), Prometheus handler.
5. **Docs**: страница shost на dvislobokov.github.io в общем стиле, README с
